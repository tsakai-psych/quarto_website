...
【追記】2026-01-20
`gt::gt()`はそのままだとHTML出力になるんですが、環境によってはフォントとかが再現されないので、画像を追加しました。

![](20260119_CFA_table_in_one_pipeline.png){.lightbox width=50% fig-align="center"}
...
`gt::gt()`と`ViewPipeSteps::print_pipe_step()`の相性が悪いので、`gt()`の前まで。
```{r}
#| code-fold: true
